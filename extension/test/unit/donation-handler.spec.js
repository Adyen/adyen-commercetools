...
describe('donation-handler::execute', () => {
  ...
  it('when response is successful add custom field action ', async () => {
    scope.post('/donations').reply(200, submitDonationsResponse)

    const ctpPaymentClone = _.cloneDeep(ctpPayment)
    ctpPaymentClone.custom.fields.donationRequest = JSON.stringify(
    ...
    expect(donationResponse.payment.merchantReference).to.equal(
