...
describe('get-payment-methods::handlePayment::', () => {
  ...
  it('handlePayment should return the right actions', async () => {
    ...
    expect(JSON.parse(result.actions[0].fields.request)).to.be.deep.includes(getPaymentMethodsRequest)
    expect(result.actions[0].fields.response).to.be.deep.equal(JSON.stringify(adyenGetPaymentResponse))
