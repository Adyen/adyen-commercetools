...
describe('submit-payment::execute', () => {
  ...
  it('when resultCode from Adyen is "Authorized", '
    ...
    ctpPaymentClone.custom.fields.submitAdditionalPaymentDetailsRequest = JSON.stringify(submitPaymentDetailsRequest)
    ...
    expect(addInterfaceInteraction.fields.type).to.equal('submitPaymentDetails')
    ...
    expect(request.reference).to.deep.equal(submitPaymentDetailsRequest.reference)
    expect(request.riskData).to.deep.equal(submitPaymentDetailsRequest.riskData)
    expect(request.paymentMethod).to.deep.equal(submitPaymentDetailsRequest.paymentMethod)
    expect(request.browserInfo).to.deep.equal(submitPaymentDetailsRequest.browserInfo)
    ...
    expect(setCustomFieldAction.name).to.equal('submitAdditionalPaymentDetailsResponse')
    ...
    expect(addTransaction.transaction.interactionId)
      .to.equal(JSON.parse(submitPaymentDetailsSuccessResponse).pspReference)
  })

  it('when resultCode from Adyen is "ChallengeShopper", '
  ...
  it('when "submitPaymentDetailsRequest" does not contain "paymentData", ' +
    'then it should add paymentData to the request', async () => {
    scope.post('/payments/details')
      .reply(200, submitPaymentDetailsChallengeResponse)
    ...
    const ctpPaymentClone = _.cloneDeep(ctpPayment)
    ctpPaymentClone.custom.fields.submitAdditionalPaymentDetailsRequest = JSON.stringify(requestWithoutPaymentData)
    ...
    const addInterfaceInteraction = response.actions.find(a => a.action === 'addInterfaceInteraction')
    const request = JSON.parse(addInterfaceInteraction.fields.request)
    expect(request.paymentData).to.equal(JSON.parse(makePaymentRedirectResponse).paymentData)
  })

  it('when "submitPaymentDetailsRequest" contains "paymentData", ' +
    'then it should NOT add paymentData to the request', async () => {
    scope.post('/payments/details')
      .reply(200, submitPaymentDetailsChallengeResponse)
