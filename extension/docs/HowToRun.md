...
In case you have a huge configuration that reaches above the environment limits (e.g the total size of all environment variables on AWS Lambda can't exceed 4 KB.), you could use the external file configuration. The extension module will look for the `.extensionrc` file in the `extension` folder. The file should contain the same content as it's defined with environment variable.
