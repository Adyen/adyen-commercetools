...
**Preconditions:**
- `makePaymentRequest` must contain a unique payment `reference` value, the reference value cannot be re-used for other commercetools payment and it's a required field by Adyen. The extension module uses reference value to set payment key, later it acts as a unique link between commercetools payment and Adyen payment. Reference may only contain alphanumeric characters, underscores, and hyphens and must have a minimum length of 2 characters and a maximum length of 80 characters. 
