...
```
{
  ...
  "adyen": {
    ...
    "adyenMerchantAccount2": {
      ...
      "ctpProjectKey": "commercetoolsProjectKey2",
      "authentication": {
        "scheme": "basic",
        "username": "xxx",
        "password": "xxx"
...
| `adyen`         | `enableBasicAuth`            | Boolean attribute. When set to "true", the [Adyen Generic Pending webhook](https://docs.adyen.com/development-resources/webhooks/webhook-types/#other-webhooks) (`eventCode: PENDING`) is registered by `npm run setup-resources` and every incoming `PENDING` notification is authenticated with [basic authentication over HTTPS](https://docs.adyen.com/development-resources/webhooks/secure-webhooks/). Generic Pending webhooks do not support HMAC signatures. Requires the `authentication` attribute. | false                                                                                                 |
| `adyen`         | `authentication`             | Basic authentication credentials of the Generic Pending webhook: `{ "scheme": "basic", "username": "xxx", "password": "xxx" }`. The same credentials are configured in Adyen when the webhook is created by `npm run setup-resources` and are validated against the `Authorization` header of every incoming `PENDING` notification. Required when `enableBasicAuth` is "true".                                                                                                                                |                                                                                                       |
| `adyen`         | `ctpProjectKey`              | Key of the commercetools project that the Adyen merchant account belongs to. Adyen Generic Pending webhooks carry no `metadata.ctProjectKey`, so `npm run setup-resources` registers the Generic Pending webhook at `<notificationBaseUrl>/notifications/<ctpProjectKey>` and incoming `PENDING` notifications are resolved to this project. Must match a project key of the `commercetools` group. Required when `enableBasicAuth` is "true".                                                                 |                                                                                                       |
...
You can create these by running the command `npm run setup-resources` as below, the command requires the `ADYEN_INTEGRATION_CONFIG` to be set as an environment variable. Be aware that this command also sets up [the notification webhook in Adyen](./IntegrationGuide.md#step-1-set-up-notification-webhook-and-generate-hmac-signature) and, when `enableBasicAuth` is "true", [the Generic Pending webhook](./IntegrationGuide.md#generic-pending-webhook-basic-authentication).
